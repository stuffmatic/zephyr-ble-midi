...
// Inspired by https://github.com/nrfconnect/sdk-nrf/blob/main/samples/bluetooth/connection_event_trigger/src/main.c
...
#define CONN_EVENT_DISTANCE_US 500

static int conn_interval_us = 0;
...
static void egu0_handler(const void *context)
{
	...
	int dur_us = conn_interval_us - CONN_EVENT_DISTANCE_US;
...
void radio_notifications_refresh_conn_interval(struct bt_conn *conn)
{
	...
	if (result) {
		int conn_interval_us = 1250 * conn_info.le.interval;
		LOG_INF("New interval %d us", conn_interval_us);
	} else {
