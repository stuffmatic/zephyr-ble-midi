...
void test_simple_running_status() {
		...
		ble_midi_packet_reset(&packet);
		packet.max_size = 100;
		ble_midi_packet_add_message(&packet, note_on_msg, 1000, 1);
		...
		ble_midi_packet_add_message(&packet, note_off_msg, 1000, 1);
...
void test_running_status_with_rt() {
		...
		ble_midi_packet_add_message(&packet, tune_request_msg, 1000, 1);
		log_packet(&packet);
		...
		ble_midi_packet_add_message(&packet, note_off_msg, 1002, 1);
		log_packet(&packet);
		test_assert(packet.size == 17, "Unexpected size");
		ble_midi_packet_add_message(&packet, tune_request_msg, 1002, 1);
		...
		log_packet(&packet);
		ble_midi_packet_add_message(&packet, note_on_msg, 1003, 1);
		log_packet(&packet);
		test_assert(packet.size == 24, "Unexpected size");
		ble_midi_packet_add_message(&packet, note_off_msg, 1003, 1);
		log_packet(&packet);
