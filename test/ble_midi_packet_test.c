#include <stdio.h>
#include <assert.h>
#include "../ble_midi/src/ble_midi_packet.h"
...
void assert_midi_msg_equals(midi_msg_t *actual, midi_msg_t *expected)
{
	...
	int equivalent = actual_high_nibble == 0x8 && expected_high_nibble == 0x9 &&
			 expected_second_data == 0;
	equivalent = equivalent || (expected_high_nibble == 0x8 && actual_high_nibble == 0x9 &&
				    actual_second_data == 0);
...
struct ble_midi_parse_cb_t ble_midi_parse_cb = {.midi_message_cb = midi_message_cb,
						.sysex_data_cb = sysex_data_cb,
						.sysex_start_cb = sysex_start_cb,
						.sysex_end_cb = sysex_end_cb};

void roundtrip_test(const char *desc, int use_running_status, midi_msg_t messages[],
		    int num_messages, uint8_t expected_payload[], int expected_payload_size)
{
	...
	for (int i = 0; i < num_messages; i++) {
		...
		if (status_byte == 0xf0) {
			assert_success(
				ble_midi_writer_start_sysex_msg(&writer, messages[i].timestamp));
		} else if (status_byte == 0xf7) {
			assert_success(
				ble_midi_writer_end_sysex_msg(&writer, messages[i].timestamp));
		} else if (status_byte < 0x80) {
			ble_midi_writer_add_sysex_data(&writer, messages[i].bytes, 1,
						       messages[i].timestamp);
		} else {
			assert_success(ble_midi_writer_add_msg(&writer, messages[i].bytes,
							       messages[i].timestamp));
		}
	}
...
void test_timestamp_byte_wrapping()
{
	...
	uint8_t expected_payload[] = {0xbe, 0xc0, 0x90, 0x69, 0x7f, 0xff, 0x80, 0x69, 0x7f, 0x80,
				      0x90, 0x69, 0x7f, 0xff, 0x80, 0x69, 0x7f, 0x83, 0x90, 0x69,
				      0x7f, 0xff, 0x80, 0x69, 0x7f, 0x80, 0x90, 0x69, 0x7f};

	roundtrip_test("Wrapping timestamp bytes should result in correct timestamps", 0, messages,
		       sizeof(messages) / sizeof(midi_msg_t), expected_payload,
...
void test_too_large_timestamp()
{
	...
	midi_msg_t msg = {.bytes = {0x90, 0x69, 0x7f}, .timestamp = 8193};
	uint8_t expected_payload[] = {0x80, 0x81, 0x90, 0x69, 0x7f};
...
void test_running_status_with_one_rt()
{
	...
	roundtrip_test("One system common message should not cancel running status", 1, messages,
		       sizeof(messages) / sizeof(midi_msg_t), expected_payload,
		       sizeof(expected_payload));
}
...
void test_running_status_with_two_rt()
{
	...
	roundtrip_test("Two consecutive real time/common messages should not cancel running status",
		       1, messages, sizeof(messages) / sizeof(midi_msg_t), expected_payload,
		       sizeof(expected_payload));
}

void test_running_status_disabled()
{
	midi_msg_t messages[] = {{.bytes = {0x90, 0x69, 0x7f}, .timestamp = 8000},
	...
	roundtrip_test("Timestamp and status bytes should be added for all messages when running "
		       "status is disabled",
		       0, messages, sizeof(messages) / sizeof(midi_msg_t), expected_payload,
		       sizeof(expected_payload));
}
...
void test_full_packet()
{
	...
	midi_msg_t messages[] = {{.bytes = {0xb0, 0x12, 0x34}, .timestamp = 10},
	...
	uint8_t expected_payload[] = {
		0x80, // packet header
		0x8a, 0xb0, 0x12, 0x34, 0x8a, 0xe0, 0x12, 0x34, 0x8a, 0xb0,
		0x12, 0x34, 0x8a, 0xe0, 0x12, 0x34, 0x8a, 0xb0, 0x12, 0x34,
...
{
	midi_msg_t messages[] = {{.bytes = {0x90, 0x69, 0x7f}, .timestamp = 100},
	...
	roundtrip_test("Sysex should cancel running status", 1, messages,
		       sizeof(messages) / sizeof(midi_msg_t), expected_payload,
		       sizeof(expected_payload));
}
...
void test_rt_in_sysex()
{
	...
	roundtrip_test("Real time message should be allowed in sysex message", 1, messages,
		       sizeof(messages) / sizeof(midi_msg_t), expected_payload,
		       sizeof(expected_payload));
}
...
void test_multi_packet_sysex()
{
	...
	int num_bytes_added =
		ble_midi_writer_add_sysex_data(&writer, sysex_data, sizeof(sysex_data), 200);
	...
	num_bytes_added = ble_midi_writer_add_sysex_data(&writer, &sysex_data[num_bytes_added],
							 sizeof(sysex_data) - num_bytes_added, 101);
...
void test_packet_end_cancels_running_status()
{
	...
	assert_success(ble_midi_writer_add_msg(&writer, note_on, 100));
	assert_success(ble_midi_writer_add_msg(&writer, note_off, 100));
	uint8_t expected_payload_2[] = {0x80, 0xe4, 0x90, 0x69, 0x7f, 0x69, 0x00};
	assert_payload_equals(&writer, expected_payload_2, sizeof(expected_payload_2));
}
...
void test_disable_note_off_as_note_on()
{
	...
	uint8_t expected_payload[] = {0x80, 0xe4, 0x90, 0x69, 0x7f, 0xe4, 0x80, 0x69, 0x7f,
				      0xe5, 0x90, 0x69, 0x7f, 0xe5, 0x80, 0x69, 0x7f};
	assert_payload_equals(&writer, expected_payload, sizeof(expected_payload));
}
...
void test_null_parse_callbacks()
{
	...
	uint8_t payload[] = {0x80, 0xe4, 0x90, 0x69, 0x7f, 0x69, 0x00, 0xe4, 0xf0,
			     0x01, 0x02, 0x03, 0xe4, 0xf7, 0xe4, 0x90, 0x69, 0x7f};
	struct ble_midi_parse_cb_t cb = {.midi_message_cb = NULL,
					 .sysex_data_cb = NULL,
					 .sysex_end_cb = NULL,
					 .sysex_start_cb = NULL};
